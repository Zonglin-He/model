"""Run Fixed-KL/B4 versus no-SSAW on the five EEG and MFD flows.

The compared branches use the same dataset-level profile, source checkpoint,
stream order, and two online inner steps.  Every cell runs in a fresh child
process and acquires the shared GPU lock to contain CUDA OOM/native failures.
"""
...
PROTOCOL = "fixed_kl_b4_eeg_mfd_five_flow_v1_steps2"
DATASETS = ("EEG", "FD")
...
INNER_STEPS = 2
DEFAULT_OUTPUT_DIR = (
    ROOT / "results" / "ablation" / "fixed_kl_b4_eeg_mfd_five_flow_steps2_v1"
)
...
def _parse_datasets(value: str | Sequence[str]) -> tuple[str, ...]:
    ...
    foreign = sorted(set(datasets) - set(DATASETS))
    if foreign:
        raise ValueError(f"unsupported datasets: {foreign}; expected EEG,FD")
    return datasets
...
def _parse_source_seeds(value: str | Sequence[int]) -> tuple[int, ...]:
    ...
    if any(seed not in SOURCE_SEEDS for seed in seeds):
        raise ValueError("formal source seeds must be drawn from 1,2,3")
    return seeds
...
def _build_specs(
    ...
    for dataset in datasets:
        ...
        base_tta_config = {**base_tta_config, "steps": INNER_STEPS}
...
def _worker(spec_path: Path) -> int:
    ...
    try:
        ...
        if lock is None:
            ...
            with lock:
                result, samples, batches = _run_fixed_kl_cell(spec)
        partition = evaluation_partition_metadata(
            str(spec["dataset"]), _flow_label(spec["flow"])
        )
        result.update(
            {
                "protocol": PROTOCOL,
...
def paired_results(raw: pd.DataFrame) -> pd.DataFrame:
    ...
    if not set(RUNNERS).issubset(pivot.columns):
        return pd.DataFrame()
    pivot = pivot.dropna(subset=list(RUNNERS)).rename(
        columns={
...
def _validate_complete(
    ...
    expected = {
        (dataset, _flow_label(flow), int(seed), runner)
        for dataset in datasets
        for flow in formal_scenario_pairs(dataset)
    ...
    for key, group in ok.groupby(
        ["dataset", "scenario", "source_seed"], dropna=False
    ):
...
def _publish(raw: pd.DataFrame, output_dir: Path) -> dict[str, object]:
    ...
    if ok.empty:
        atomic_write_csv(
            pd.DataFrame(
                columns=["dataset", "scenario", "runner", "source_seeds", "f1_mean", "f1_std"]
            ),
            output_dir / "flow_summary.csv",
            index=False,
        )
        atomic_write_csv(
            pd.DataFrame(columns=["dataset", "runner", "units", "f1_mean", "f1_std"]),
            output_dir / "method_summary.csv",
            index=False,
        )
        summary = {"status": "incomplete", "paired_units": 0}
        atomic_write_json(summary, output_dir / "paired_summary.json")
        return summary
    flow_summary = (
        ok.groupby(["dataset", "scenario", "runner"], dropna=False)
        .agg(
    ...
    method_summary = (
        ok.groupby(["dataset", "runner"], dropna=False)
    ...
    if paired.empty:
        summary: dict[str, object] = {"status": "incomplete"}
    else:
        datasets = {}
...
def _run_parent(args) -> int:
    ...
    specs = _build_specs(args, datasets, source_seeds)
    manifest = {
        "protocol": PROTOCOL,
        "status": "running",
        "datasets": list(datasets),
        "flows": {
            dataset: [_flow_label(flow) for flow in formal_scenario_pairs(dataset)]
            for dataset in datasets
        },
        ...
        "profile_shared_across_all_flows_within_dataset": True,
            ...
            dataset: {
                ...
                "tta_configs": {
                    runner: next(
                        spec["tta_config"]
                        for spec in specs
                        if spec["dataset"] == dataset and spec["runner"] == runner
                    )
                    for runner in RUNNERS
                },
            }
    ...
    raw = _collect(specs)
    validation = _validate_complete(raw, datasets, source_seeds)
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--datasets", default="EEG,FD")
