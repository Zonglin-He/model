"""Run a one-factor-at-a-time SSAW internal ablation on HAR source seed 1."""
...
PROTOCOL = "har_ssaw_internal_ablation_seed1_v1"
...
DEFAULT_OUTPUT_DIR = (
    ROOT / "results" / "ablation" / "har_ssaw_internal_ablation_seed1_v1"
)
...
def _publish(raw: pd.DataFrame, output_dir: Path) -> dict[str, object]:
    ...
    summary = (
        ...
        .sort_values("removal_effect_mean", ascending=False)
    )
    ...
    return {
        "status": "complete" if len(ok) == len(FLOWS) * len(RUNNERS) else "incomplete",
